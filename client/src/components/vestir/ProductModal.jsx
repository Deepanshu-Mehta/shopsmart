...
export default function ProductModal({ product, onClose, onAddToCart }) {
  ...
  const handleClose = () => { setOpen(false); setTimeout(onClose, 500); document.body.style.overflow = ''; };
      ...
      >
        ...
        <div style={{
          ...
          <button
            ...
            style={{ fontSize: 20, fontWeight: 300, color: 'var(--color-ink)', transition: 'color 250ms' }}
        ...
        <div style={{ padding: 32, flex: 1 }}>
          ...
          <div role="group" aria-label="Size selector" style={{ display: 'flex', gap: 8, marginBottom: 32, flexWrap: 'wrap' }}>
            {SIZES.map(s => (
              <button
                key={s}
                data-hover
          ...
          {/* Color */}
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--color-muted)', marginBottom: 12 }}>Colour</p>
          <div role="group" aria-label="Colour selector" style={{ display: 'flex', gap: 12, marginBottom: 32 }}>
            {COLORS.map(c => (
              <button
                key={c.label}
                data-hover
          ...
          {/* Quantity */}
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--color-muted)', marginBottom: 12 }}>Quantity</p>
          <div role="group" aria-label="Quantity" style={{ display: 'flex', alignItems: 'center', border: '1px solid rgba(26,25,22,0.12)', width: 'fit-content', marginBottom: 24 }}>
            <button data-hover onClick={() => setQty(q => Math.max(1, q - 1))} style={{ width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 300, color: 'var(--color-ink)' }} aria-label="Decrease quantity" className="qty-btn">−</button>
            <span aria-live="polite" style={{ width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-body)', fontSize: 14, borderLeft: '1px solid rgba(26,25,22,0.12)', borderRight: '1px solid rgba(26,25,22,0.12)' }}>{qty}</span>
            <button data-hover onClick={() => setQty(q => q + 1)} style={{ width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 300, color: 'var(--color-ink)' }} aria-label="Increase quantity" className="qty-btn">+</button>
          </div>
          ...
          {accordions.map(a => (
            <div key={a.key} style={{ borderTop: '1px solid rgba(26,25,22,0.12)' }}>
              ...
              <div
                className={`accordion-body${accordionOpen === a.key ? ' open' : ''}`}
              >
                <p style={{
                  fontFamily: 'var(--font-body)',
                  fontSize: 13,
